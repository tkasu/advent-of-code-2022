...
object part1 extends IOApp.Simple {
  ...
  case class RPCRound(otherInput: RPCInput, myInput: RPCInput) {
    ...
    def perfScore = this match {
      ...
      case RPCRound(RPCInput.Rock, my) => my match {
        case RPCInput.Paper => 6
        case RPCInput.Scissors => 0
      }
      case RPCRound(RPCInput.Paper, my) => my match {
        case RPCInput.Scissors => 6
        case RPCInput.Rock => 0
      }
      case RPCRound(RPCInput.Scissors, my) => my match {
  ...
  object RPCRound:
    ...
    /**
     * Parse input from AoC input format:
     * e.g. 'A X'
     * First other players is my input:
     * A -> Rock
     * B -> Paper
     * C -> Scissors
     * After space, the second latter is my input:
     * X -> Rock
     * Y -> Paper
     * Z -> Scissors
     * */
